...
namespace fc {
    ...
    public_key::public_key( const string& pem_file )
    :my( std::make_shared<detail::pke_impl>() )
    {
        BIO* mem = BIO_new(BIO_s_file()); 
        BIO_read_filename(mem, pem_file.c_str()); 

        my->rsa = PEM_read_bio_RSA_PUBKEY(mem, NULL, NULL, NULL); 
        BIO_free_all(mem);

        FC_ASSERT( my->rsa, "read public key" );
    }
    ...
    :my( std::make_shared<detail::pke_impl>() )
    {
       ...
       auto b64 = string(d.begin(), d.end());
       //fc::base64_encode( (const unsigned char*)d.data(), d.size() );
    ...
    {
       FC_ASSERT( sig.size() == 2048/8, "Invalid signature length" );
       return 0 != RSA_verify( NID_sha1, (const uint8_t*)&digest, 20,
    ...
    private_key::private_key( const string& pem_file )
    :my( std::make_shared<detail::pke_impl>() )
    {
        BIO* mem = BIO_new(BIO_s_file()); 
        BIO_read_filename(mem, pem_file.c_str()); 
